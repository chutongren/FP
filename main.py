...
def plot_all_peaks(filtered_data, fs, spikes_selected, channel_num, dataset_num):
    ...
    for i, peak_index in enumerate(spikes_selected):
        if peak_index - pre_peak_samples >= 0 and peak_index + post_peak_samples < len(filtered_data):
            spike_segment = filtered_data[peak_index - pre_peak_samples : peak_index + post_peak_samples]
            spike_data.append(spike_segment)  # 将当前spike的采样段保存到二维数组中
...
def wavelet_transform(spike_data):
    wavelet_features = []
    for spike in spike_data:
        coeffs = pywt.wavedec(spike, 'db16', level=8)
        wavelet_features.append(np.concatenate(coeffs))
    return np.array(wavelet_features)

def select_best_features(wavelet_features, alpha=0.05):
    selected_features = []
    for i in range(wavelet_features.shape[1]):
        feature_column = wavelet_features[:, i]
        D, p_value = kstest(feature_column, 'norm')
        if p_value < alpha:
            selected_features.append(feature_column)
    print(len(selected_features))
    return np.array(selected_features).T

def plot_wavelet_clusters(spike_data, selected_features, n_clusters=2):
    # demention reduction to 2D
    pca = PCA(n_components=2)
    reduced_features = pca.fit_transform(selected_features)

    kms = KMeans(n_clusters=n_clusters)
    labels = kms.fit_predict(reduced_features)
    
    # create DataFrame
    result_df = pd.DataFrame(reduced_features, columns=['PCA1', 'PCA2'])
    result_df['cluster'] = labels
    
    cluster_colors = {i: color for i, color in enumerate(['blue', 'red', 'green', 'black', 'yellow'])}

    # scatter plot: hier only the first two featurs
    plt.figure()
    for cluster in range(n_clusters):
        cluster_data = result_df[result_df['cluster'] == cluster]
        plt.scatter(cluster_data['PCA1'], cluster_data['PCA2'], color=cluster_colors[cluster], label=f'Cluster {cluster+1}')
    
    plt.title('Wavelet Feature Clusters (PCA Reduced)')
    plt.xlabel('PCA1')
    plt.ylabel('PCA2')
    plt.legend()
    plt.show()

    return cluster_colors, labels

def plot_representative_waveforms(spike_data, cluster_labels, fs, cluster_colors):
    t = np.arange(0, spike_data.shape[1]) / fs * 1000  # 时间轴(ms)
    ...
    for cluster_id in cluster_ids:
        ...
        if len(cluster_indices) > 0:
            representative_spike = spike_data[cluster_indices[0]]
            plt.plot(t, representative_spike, color=cluster_colors[cluster_id], label=f'Cluster {cluster_id+1}')
    
            plt.title('Representative Waveforms for Each Cluster')
            plt.xlabel('Time (ms)')
            plt.ylabel('Voltage (μV)')
            plt.legend()
            plt.show()
...
if __name__ == '__main__':
    ...
    peaks = find_peaks_in_data(filtered_data[7, :], peak_height=50) # ‘peak_height’ requires manual input, amp(spikes) > peak_height will be selected
    print(len(peaks))
    
    spikes_selected = peaks

    spike_data = plot_all_peaks(filtered_data[7,:], fs, spikes_selected=spikes_selected, channel_num=channel_selected, dataset_num=dataset_num)
    
    
    # Step 1: feature extraction using DWT
    wavelet_features = wavelet_transform(spike_data)

    # Step 2: Select the best features using Kolmogorov-Smirnov test
    selected_features = select_best_features(wavelet_features, alpha=0.05)

    # Step 3: Use KMeans to perform clustering and plot the results
    cluster_colors, labels = plot_wavelet_clusters(spike_data, selected_features, n_clusters=2)
    plot_representative_waveforms(spike_data, labels, fs=30000, cluster_colors=cluster_colors)
